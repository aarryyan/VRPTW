...
class Problem:
    def __init__(self, name, customers: list, vehicle_number, vehicle_capacity):
        ...
        self.depot = next((customer for customer in customers if customer.number == 0), None)
        if self.depot:
    ...
    def print_canonical(self, routes):
        return "\n".join(route.canonical_view for route in routes)


class Route:
    def __init__(self, problem: Problem, customers: list):
        self.problem = problem
        self._customers = [self.problem.depot, *customers, self.problem.depot]
